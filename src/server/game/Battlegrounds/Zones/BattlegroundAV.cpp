...
void BattlegroundAV::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    UpdateScore(player->GetBgTeamId(), -1);
}
...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    if (entry == BG_AV_CreatureInfo[AV_NPC_A_BOSS])
    {
		...
		RewardReputationToTeam(729, 730, BG_AV_REP_BOSS, killer->GetBgTeamId() == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN])
    {
		...
		RewardReputationToTeam(729, 730, BG_AV_REP_CAPTAIN, killer->GetBgTeamId() == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN])
    {
        ...
        ChangeMineOwner(AV_NORTH_MINE, killer->GetBgTeamId());
        ...
        ChangeMineOwner(AV_SOUTH_MINE, killer->GetBgTeamId());
}
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
	...
	TeamId CFSteam = GetOtherTeam(player->GetBgTeamId());
    ...
    switch (questid)
    {
        ...
        case AV_QUEST_H_SCRAPS2:
            ...
            if (m_Team_QuestStatus[teamId][0] == 500 || m_Team_QuestStatus[teamId][0] == 1000 || m_Team_QuestStatus[teamId][0] == 1500) //25, 50, 75 turn ins
            {
                ...
                for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT; ++i)
                    if (m_Nodes[i].OwnerId == player->GetBgTeamId() && m_Nodes[i].State == POINT_CONTROLED)
                    {
                        DePopulateNode(i);
                        PopulateNode(i);
        ...
        case AV_QUEST_H_COMMANDER1:
			...
			RewardReputationToTeam(teamId, CFSteam, 1, player->GetBgTeamId());
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    if (ownerId == player->GetBgTeamId() || m_Nodes[node].State != POINT_ASSAULTED)
        return;
...
WorldSafeLocsEntry const* BattlegroundAV::GetClosestGraveyard(Player* player)
{
    ...
    pGraveyard = sWorldSafeLocsStore.LookupEntry(BG_AV_GraveyardIds[player->GetBgTeamId()+7]);
