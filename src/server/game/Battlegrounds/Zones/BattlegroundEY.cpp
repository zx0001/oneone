		...
		for (uint8 point = 0; point < EY_POINTS_MAX; ++point)
			if (GameObject* pointObject = pointsVec[point])
				if (itr->second->CanCaptureTowerPoint() && itr->second->IsWithinDistInMap(pointObject, BG_EY_POINT_RADIUS))
				{
					...
					++_capturePointInfo[point]._playersCount[itr->second->GetBgTeamId()];
...
void BattlegroundEY::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    switch (trigger)
    {
        case AT_BLOOD_ELF_POINT:
			if (_capturePointInfo[POINT_BLOOD_ELF].IsUnderControl(player->GetBgTeamId()))
        ...
        case AT_FEL_REAVER_POINT:
			if (_capturePointInfo[POINT_FEL_REAVER].IsUnderControl(player->GetBgTeamId()))
                if (_flagState == BG_EY_FLAG_STATE_ON_PLAYER && GetFlagPickerGUID() == player->GetGUID())
                    EventPlayerCapturedFlag(player, BG_EY_OBJECT_FLAG_FEL_REAVER);
            break;
        case AT_MAGE_TOWER_POINT:
			if (_capturePointInfo[POINT_MAGE_TOWER].IsUnderControl(player->GetBgTeamId()))
                if (_flagState == BG_EY_FLAG_STATE_ON_PLAYER && GetFlagPickerGUID() == player->GetGUID())
                    EventPlayerCapturedFlag(player, BG_EY_OBJECT_FLAG_MAGE_TOWER);
            break;
        case AT_DRAENEI_RUINS_POINT:
			if (_capturePointInfo[POINT_DRAENEI_RUINS].IsUnderControl(player->GetBgTeamId()))
                if (_flagState == BG_EY_FLAG_STATE_ON_PLAYER && GetFlagPickerGUID() == player->GetGUID())
...
void BattlegroundEY::EventPlayerDroppedFlag(Player* player)
{
	...
	SendMessageToAll(LANG_BG_EY_DROPPED_FLAG, player->GetBgTeamId() == TEAM_ALLIANCE ? CHAT_MSG_BG_SYSTEM_ALLIANCE : CHAT_MSG_BG_SYSTEM_HORDE);
}
...
void BattlegroundEY::EventPlayerClickedOnFlag(Player* player, GameObject* gameObject)
{
	...
	PSendMessageToAll(LANG_BG_EY_HAS_TAKEN_FLAG, player->GetBgTeamId() == TEAM_ALLIANCE ? CHAT_MSG_BG_SYSTEM_ALLIANCE : CHAT_MSG_BG_SYSTEM_HORDE, NULL, player->GetName().c_str());
	PlaySoundToAll(player->GetBgTeamId() == TEAM_ALLIANCE ? BG_EY_SOUND_FLAG_PICKED_UP_ALLIANCE : BG_EY_SOUND_FLAG_PICKED_UP_HORDE);
...
void BattlegroundEY::EventPlayerCapturedFlag(Player* player, uint32 BgObjectType)
{
    ...
    if (_ownedPointsCount[player->GetBgTeamId()] > 0)
        AddPoints(player->GetBgTeamId(), BG_EY_FlagPoints[_ownedPointsCount[player->GetBgTeamId()] - 1]);
}
...
{
    WorldSafeLocsEntry const* entry = sWorldSafeLocsStore.LookupEntry(BG_EY_GRAVEYARD_MAIN_ALLIANCE + player->GetBgTeamId());
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
		if (_capturePointInfo[i].IsUnderControl(player->GetBgTeamId()))
        {
